...
def compute_embedding(model, node_csv_without_emb, node_csv_file, edge_csv_without_emb, edge_csv_file, text_node_csv, text_node_csv_with_emb):
            ...
            for row in reader_node:
                ...
                if len(batch_nodes) == batch_size:
                    ...
                    for row in batch_rows:
                        row[5] = node_embedding_dict[row[0]].tolist()
        ...
        with open(edge_csv_file, "w") as csvfile_edge_emb:
            ...
            for row in reader_edge:
                ...
                if len(batch_edges) == batch_size:
                    ...
                    for row in batch_rows:
                        row[6] = edge_embedding_dict[";".join([row[0], row[2], row[1]])].tolist()
                        writer_edge.writerow(row)
                    batch_edges = []
                    batch_rows = []
    ...
    with open(text_node_csv, "r") as csvfile_text_node:
        with open(text_node_csv_with_emb, "w") as csvfile_text_node_emb:
            ...
            batch_text_nodes = []
            batch_rows = []
            for row in tqdm(reader_text_node):
                if row[0] == "text_id:ID":
                    continue
                ...
                if len(batch_text_nodes) == batch_size:
                    ...
                    text_node_embeddings = model.encode_multi_process(batch_text_nodes, pool=pool, batch_size=batch_size, show_progress_bar=False)
